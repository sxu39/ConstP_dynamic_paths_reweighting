...
int Input::execute_command()
{
  ...
  else if (mycmd == "Ne_dot") Ne_dot();//?
  else if (mycmd == "dE_dN") dE_dN();//?
...
void Input::Ne()
{
  if (narg != 1) error->all(FLERR,"Illegal Ne command: expected 1 arguments but found {}", narg);
  atom->set_Ne(FLERR,narg,arg);
}
...
void Input::Ne_dot()
{
  if (narg != 1) error->all(FLERR,"Illegal Ne_dot command: expected 1 arguments but found {}", narg);
  atom->set_Ne_dot(FLERR,narg,arg);
}
...
void Input::dE_dN()
{
  if (narg != 1) error->all(FLERR,"Illegal dE_dN command: expected 1 arguments but found {}", narg);
  atom->set_dE_dN(FLERR,narg,arg);
}
