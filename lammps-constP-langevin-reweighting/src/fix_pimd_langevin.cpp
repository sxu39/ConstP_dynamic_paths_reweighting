...
FixPIMDLangevin::FixPIMDLangevin(LAMMPS *lmp, int narg, char **arg) :
    ...
    eta(nullptr), eta_dot(nullptr), eta_dotdot(nullptr), eta_mass(nullptr), Ne_dot(nullptr), Ne_mass(nullptr) //?
{
  restart_global = 1;
  time_integrate = 1;
  ...
  for (int i = 3; i < narg - 1; i += 2) {
    if (strcmp(arg[i], "method") == 0) {
      if (strcmp(arg[i + 1], "nmpimd") == 0)
        ...
        method = PIMD;
      else if (strcmp(arg[i + 1], "tprpmd") == 0) //?TPRPMD
        method = TPRPMD;
      else
      ...
      if (strcmp(arg[i + 1], "PILE_L") == 0) {
        ...
        mu = utils::numeric(FLERR, arg[i + 1], false, lmp);
    } else if (strcmp(arg[i], "Udamp") == 0) {  //?
...
void FixPIMDLangevin::setup(int vflag)
{
  ...
  if (method == NMPIMD || method == TPRPMD) {  //?TPRPMD
...
void FixPIMDLangevin::b_step()
{
  ...
  if (ustat_flag) {
    ...
    double dE_dN_k = *dE_dN, dE_dN_ave = 0;
    MPI_Barrier(universe->uworld);
    MPI_Allreduce(&dE_dN_k, &dE_dN_ave, 1, MPI_DOUBLE, MPI_SUM, universe->uworld);
    MPI_Barrier(universe->uworld);
    dE_dN_ave *= inverse_np;
    MPI_Bcast(&dE_dN_ave, 1, MPI_DOUBLE, 0, universe->uworld);
    *Ne_dot += dtfm * (-dE_dN_ave + mu);  //?
  }
}
...
void FixPIMDLangevin::nhc_init()
{
  ...
  if (thermostat == NHC) {
    if (tstat_flag) {
      ...
      if (method == TPRPMD && np!=1 && universe->iworld == 0){  //?TPRPMD
        for (int ich = 1; ich < mtchain; ich++) {
          eta_dotdot[ich] = 0;
        }
      }
...
void FixPIMDLangevin::nhc_temp_integrate()
{
  ...
  if (method == TPRPMD && np!=1 && universe->iworld == 0) return; //?TPRPMD
...
void FixPIMDLangevin::nhc_mu_integrate() //?
{
  int ich;
  double expfac;
  int *type = atom->type;
  ...
  if (method == TPRPMD && np!=1 && universe->iworld == 0) //?TPRPMD
    ...
    if (eta_mass[0] > 0.0)
      if (method == TPRPMD && np!=1) //?TPRPMD
        eta_dotdot[0] = (kecurrent + (*Ne_mass)*(*Ne_dot)*(*Ne_dot)/(static_cast<double>(np) - 1) - np * (tdof+inverse_np) * force->boltz * temp)/eta_mass[0];
      else
        eta_dotdot[0] = (kecurrent + inverse_np*(*Ne_mass)*(*Ne_dot)*(*Ne_dot) - np * (tdof+inverse_np) * force->boltz * temp)/eta_mass[0];
    else eta_dotdot[0] = 0.0;
  }
...
void FixPIMDLangevin::nmpimd_init()
{
  ...
  // std::cout << "nmpimd_init()" << universe->iworld << std::endl;
}
...
double FixPIMDLangevin::compute_vector(int n)
{
  ...
  if (thermostat == NHC) {
    ...
    if (ustat_flag) { 
      ...
      n -= ilen;  
      ilen = 1;
      if (n < ilen) return kt * eta[0] - mu*(*Ne);
      n -= ilen;
    }
  }
