...
void FixLangevin::init()
{
  ...
  if (!atom->rmass) {
    ...
    if (ustat_flag) {
      double gamma_ref = 0.1;
      gfactor1[0] = -*Ne_mass / (t_period / gamma_ref) / force->ftm2v;
      gfactor2[0] = sqrt(*Ne_mass) / force->ftm2v;
      gfactor2[0] *= sqrt(24.0 * force->boltz / (t_period / gamma_ref) / update->dt / force->mvv2e) ;
    }
  }
...
void FixLangevin::post_force_templated()
{
  ...
  int *mask = atom->mask;
  int nlocal = atom->nlocal;
  // NOTE: ustat_flag indicates additional scalar-like stochastic term usage.
  // Access any such fields directly via atom->Ne_dot and atom->dE_dN at use-sites
  // to avoid scope/shadowing issues.
...
//  int test_id = 152;
//  std::cout << "----------langevin force update ----------" << std::endl;

//  std::cout << "f_before:" << f[test_id][0] << " " << f[test_id][1] << " " << f[test_id][2] << std::endl;
...
//  std::cout << "f_after:" << f[test_id][0] << " " << f[test_id][1] << " " << f[test_id][2] << std::endl;
  ...
  if (ustat_flag) {
//    std::cout << "dE_dN before: " << *(atom->dE_dN) << std::endl;

    double fran_Ne, fdrag_Ne;
    fran_Ne = gfactor2[0]*tsqrt*(random->uniform()-0.5);
    fdrag_Ne = gfactor1[0]*(*(atom->Ne_dot));
    *(atom->dE_dN) += fran_Ne + fdrag_Ne;

//    std::cout << "dE_dN after: " << *(atom->dE_dN) << std::endl;
  }
}
...
void FixLangevin::reset_dt()
{
  if (atom->mass) {
    ...
    if (ustat_flag) {
      gfactor1[0] = -*Ne_mass / t_period / force->ftm2v;
      gfactor2[0] = sqrt(*Ne_mass) / force->ftm2v;
      gfactor2[0] *= sqrt(24.0 * force->boltz / t_period / update->dt / force->mvv2e);
    }
  }
  if (gjfflag) {
    gjfa = (1.0 - update->dt / 2.0 / t_period) / (1.0 + update->dt / 2.0 / t_period);
