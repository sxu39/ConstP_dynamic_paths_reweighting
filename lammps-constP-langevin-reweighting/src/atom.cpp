...
/* ----------------------------------------------------------------------
   set Ne and flag them as set
   called from reading of input script
------------------------------------------------------------------------- */

void Atom::set_Ne(const char *file, int line, int /*narg*/, char **arg)
{
  Ne = new double[1];

  const double value = utils::numeric(FLERR, arg[0], false, lmp);

  *Ne = value;
}

/* ----------------------------------------------------------------------
   set Ne_dot and flag them as set
   called from reading of input script
------------------------------------------------------------------------- */

void Atom::set_Ne_dot(const char *file, int line, int /*narg*/, char **arg)
{
  Ne_dot = new double[1];
...
/* ----------------------------------------------------------------------
   set dE/dN and flag them as set
   called from reading of input script
------------------------------------------------------------------------- */

void Atom::set_dE_dN(const char *file, int line, int /*narg*/, char **arg)
{
  dE_dN = new double[1];
...
void Atom::data_Ne_dot(const char *file, int line, const char *str)
{
  Ne_dot = new double[1];
  double Ne_dot_one;
  auto location = "Ne_dot section of data file";
  auto values = Tokenizer(str).as_vector();
  int nwords = values.size();
  for (std::size_t i = 0; i < values.size(); ++i) {
    if (utils::strmatch(values[i], "^#")) {
  ...
  Ne_dot_one = utils::numeric(file, line, values[0], false, lmp);
