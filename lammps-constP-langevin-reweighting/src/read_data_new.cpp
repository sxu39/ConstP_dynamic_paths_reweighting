...
bool ReadDataNew::is_data_section(const std::string &keyword)
{
...
// clang-format on
/* ---------------------------------------------------------------------- */
ReadDataNew::ReadDataNew(LAMMPS *_lmp) : Command(_lmp), fp(nullptr), coeffarg(nullptr), lmap(nullptr)
{
...
void ReadDataNew::command(int narg, char **arg)
{
  ...
  while (true) {
    ...
    while (strlen(keyword)) {
      ...
      } else if (strcmp(keyword, "Electron Number") == 0) {  //?
        if (firstpass)
          electronnumber();
        else
          skip_lines(natoms);

      } else if (strcmp(keyword, "dE/dN") == 0) {
        if (firstpass)
          dE_dN();
        else
          skip_lines(natoms);

      } else if (strcmp(keyword, "Ne_dot") == 0) {
        if (firstpass)
          Ne_dot();
        else
          skip_lines(natoms);
...
void ReadDataNew::header(int firstpass)
{
  int n;
  char *ptr;
...
void ReadDataNew::electronnumber()  //?
{
  char *next;
  auto buf = new char[1 * MAXLINE];

  if (me == 0) utils::logmesg(lmp, "  reading electron number ...\n");

  int eof = utils::read_lines_from_file(fp, 1, MAXLINE, buf, me, world);
  if (eof) error->all(FLERR, "Unexpected end of data file");

  char *original = buf;
  next = strchr(buf, '\n');
  if (!next) error->all(FLERR, "Missing data in Electron Number section of data file");
  *next = '\0';
  atom->data_electronnumber(FLERR, buf);

  if (me == 0) utils::logmesg(lmp, "  {} electron number\n", natoms);
  delete[] original;
}
...
void ReadDataNew::dE_dN()  //?
{
  char *next;
  auto buf = new char[1 * MAXLINE];

  if (me == 0) utils::logmesg(lmp, "  reading dE/dN ...\n");
  ...
  char *original = buf;
  next = strchr(buf, '\n');
  if (!next) error->all(FLERR, "Missing data in dE/dN section of data file");
  *next = '\0';
  atom->data_dE_dN(FLERR, buf);

  if (me == 0) utils::logmesg(lmp, "  {} dE/dN\n", natoms);
  delete[] original;
}
...
void ReadDataNew::Ne_dot()  //?
{
  char *next;
  auto buf = new char[1 * MAXLINE];

  if (me == 0) utils::logmesg(lmp, "  reading Ne_dot ...\n");
  ...
  char *original = buf;
  next = strchr(buf, '\n');
  if (!next) error->all(FLERR, "Missing data in Ne_dot section of data file");
  *next = '\0';
  atom->data_Ne_dot(FLERR, buf);

  if (me == 0) utils::logmesg(lmp, "  {} Ne_dot\n", natoms);
  delete[] original;
}
...
void ReadDataNew::bonds(int firstpass)
{
  int nchunk, eof;
...
void ReadDataNew::angles(int firstpass)
{
  int nchunk, eof;
...
void ReadDataNew::dihedrals(int firstpass)
{
  int nchunk, eof;
...
void ReadDataNew::impropers(int firstpass)
{
  int nchunk, eof;
...
void ReadDataNew::bonus(bigint nbonus, AtomVec *ptr, const char *type)
{
  int nchunk, eof;
...
void ReadDataNew::bodies(int firstpass, AtomVec *ptr)
{
...
void ReadDataNew::anglecoeffs(int which)
{
...
void ReadDataNew::dihedralcoeffs(int which)
{
...
void ReadDataNew::impropercoeffs(int which)
{
...
void ReadDataNew::typelabels(int mode)
{
...
void ReadDataNew::fix(Fix *ifix, char *keyword)
{
  int nchunk, eof;
...
int ReadDataNew::reallocate(int **pcount, int cmax, int amax)
{
  int *count = *pcount;
...
void ReadDataNew::open(const std::string &file)
{
...
void ReadDataNew::parse_keyword(int first)
{
  int eof = 0;
  int done = 0;
...
void ReadDataNew::skip_lines(bigint n)
{
  if (me) return;
  if (n <= 0) return;
  char *eof = nullptr;
...
void ReadDataNew::parse_coeffs(char *line, const char *addstr, int dupflag, int noffset, int offset,
...
int ReadDataNew::style_match(const char *one, const char *two)
{
