  ...
  nwall(0)
{
  ...
  while (iarg < narg) {
    if ((strcmp(arg[iarg],"xlo") == 0) || (strcmp(arg[iarg],"xhi") == 0) ||
        ...
        (strcmp(arg[iarg],"Nelo") == 0) || (strcmp(arg[iarg],"Nehi") == 0)) {
      ...
      else if (strcmp(arg[iarg],"Nelo") == 0) newwall = NeLO;
      else if (strcmp(arg[iarg],"Nehi") == 0) newwall = NeHI;

      for (int m = 0; (m < nwall) && (m < 8); m++)
        if (newwall == wallwhich[m])
          error->all(FLERR,"Wall defined twice in fix wall/reflect command");

      wallwhich[nwall] = newwall;
      if (strcmp(arg[iarg+1],"EDGE") == 0) {
        if (newwall == NeLO || newwall == NeHI)
          error->all(FLERR,"Ne walls do not support the EDGE keyword");
  ...
  if (flag) {
    ...
    for (int m = 0; m < nwall; m++) {
      ...
      if (wallwhich[m] == NeLO || wallwhich[m] == NeHI) continue;
...
void FixWallReflect::post_integrate()
{
  ...
  for (int m = 0; m < nwall; m++) {
    if (wallstyle[m] == VARIABLE) {
      ...
      if (wallwhich[m] == NeLO || wallwhich[m] == NeHI) {
        // Ne is a scalar state variable, not a length, so do not rescale it.
      } else if (wallwhich[m] < YLO) coord *= xscale;
...
void FixWallReflect::wall_particle(int /* m */, int which, double coord)
{
  ...
  if (which == NeLO || which == NeHI) {
    if (!atom->Ne) error->all(FLERR,"Fix wall/reflect Ne wall requires atom Ne to be defined");
